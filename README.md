...
```sh
flowpipe pipeline run create_card --pipeline-arg list_id="LIST_ID" --pipeline-arg card_name="CARD_NAME"
```

### Credentials

This mod uses the credentials configured in `flowpipe.pvars` or passed through `--pipeline-args token` and `--pipeline-args api_key`.
...
- Pass in a value on the command line:
  ...
  ```shell
  flowpipe pipeline run create_card --pipeline-arg list_id="LIST_ID" --pipeline-arg card_name="CARD_NAME" --pipeline-arg token="TOKEN" --pipeline-arg api_key="API_KEY"
  ```
